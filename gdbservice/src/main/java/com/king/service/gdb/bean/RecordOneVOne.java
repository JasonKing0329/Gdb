package com.king.service.gdb.bean;

public class RecordOneVOne extends RecordSingleScene {

	private Star star1;
	private Star star2;
	private int star1Id;
	private int star2Id;
	private int scoreStar1;
	private int scoreStar2;
	private int scoreStarC1;
	private int scoreStarC2;
	private int scoreFkType1;
	private int scoreFkType2;
	private int scoreFkType3;
	private int scoreFkType4;
	private int scoreFkType5;
	private int scoreFkType6;
	public Star getStar1() {
		return star1;
	}
	public void setStar1(Star star1) {
		this.star1 = star1;
	}
	public Star getStar2() {
		return star2;
	}
	public void setStar2(Star star2) {
		this.star2 = star2;
	}

	public int getStar1Id() {
		return star1Id;
	}

	public void setStar1Id(int star1Id) {
		this.star1Id = star1Id;
	}

	public int getStar2Id() {
		return star2Id;
	}

	public void setStar2Id(int star2Id) {
		this.star2Id = star2Id;
	}

	public int getScoreStar1() {
		return scoreStar1;
	}
	public void setScoreStar1(int scoreStar1) {
		this.scoreStar1 = scoreStar1;
	}
	public int getScoreStar2() {
		return scoreStar2;
	}
	public void setScoreStar2(int scoreStar2) {
		this.scoreStar2 = scoreStar2;
	}
	public int getScoreStarC1() {
		return scoreStarC1;
	}
	public void setScoreStarC1(int scoreStarC1) {
		this.scoreStarC1 = scoreStarC1;
	}
	public int getScoreStarC2() {
		return scoreStarC2;
	}
	public void setScoreStarC2(int scoreStarC2) {
		this.scoreStarC2 = scoreStarC2;
	}
	public int getScoreFkType1() {
		return scoreFkType1;
	}
	public void setScoreFkType1(int scoreFkType1) {
		this.scoreFkType1 = scoreFkType1;
	}
	public int getScoreFkType2() {
		return scoreFkType2;
	}
	public void setScoreFkType2(int scoreFkType2) {
		this.scoreFkType2 = scoreFkType2;
	}
	public int getScoreFkType3() {
		return scoreFkType3;
	}
	public void setScoreFkType3(int scoreFkType3) {
		this.scoreFkType3 = scoreFkType3;
	}
	public int getScoreFkType4() {
		return scoreFkType4;
	}
	public void setScoreFkType4(int scoreFkType4) {
		this.scoreFkType4 = scoreFkType4;
	}
	public int getScoreFkType5() {
		return scoreFkType5;
	}
	public void setScoreFkType5(int scoreFkType5) {
		this.scoreFkType5 = scoreFkType5;
	}
	public int getScoreFkType6() {
		return scoreFkType6;
	}
	public void setScoreFkType6(int scoreFkType6) {
		this.scoreFkType6 = scoreFkType6;
	}

}
